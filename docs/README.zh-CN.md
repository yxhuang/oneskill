...
- 冲突的东西只改名，不删除。工具目录里那份副本，会挪出该工具的加载路径，放到
  ...
  备份；
- 唯一的例外是 `osk update`：更新成功后，它会把刚替换下来的那份备份删掉（要留就加
  `--keep-backups N`）。remote skill 的 manifest 记着 `source_url` 和 `ref`，旧版随时能从
  上游取回，而留着每一份历史本体会让大 skill 每更新一次就在磁盘上翻一倍。更新过程中备份
  照样会建，失败时就是靠它回滚；
