...
已有技能会显示为 `unmanaged`(未纳管)。按 `osk doctor` 给出的命令逐个收编即可——
`osk adopt <路径>` 会把本体移入库中、原位换成软链、并链入其余客户端。如果你曾把
同一个技能手工复制到两个客户端,一次 `adopt` 就会把它们合而为一(多余的副本会
改名备份,绝不删除)。
...
| `osk adopt <路径>` | 把住在某一端的技能收编进库,共享给其余客户端 |
...
- **冲突一律改名,绝不删除**——挡路的东西会变成
  `<名字>.oneskill-backup-<时间戳>`,整个代码库里没有一个 `rmtree`;
...
**我的技能已经散落各处,上手有多痛苦?**
`osk scan --write` 一次盘点全部;`osk doctor` 为每个未纳管技能打出一条 `adopt`
命令。同一技能的多端手工副本会在第一次 `adopt` 时合并为一份共享本体。
...
年轻,但对此很诚实:安全性质有 31 个离线测试背书,CI 覆盖 Linux/macOS ×
...
路线图:批量收编 `adopt --all`(一条命令完成上手)、更多客户端支持、以及基于
`osk list --json` 的 GUI(JSON 键已稳定,可放心对接)。
